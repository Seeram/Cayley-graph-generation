...
We present algorithm for checking diameter of Cayley graph in psedocode. As an input procedure takes {\em generating set} and {\em diameter} we want check on graph. First we put all elements of {\em generating set} into {\em reached nodes}. Next loop iterates over all $n \in \{2 \dots diameter\}$. Inner loop iterates over all variations with repetition of indeces of length $n$ from {\em gen\_set}. We assign each variation to array {\em var} and multiply {\em gen\_set} elements in same order as saved indeces in {\em var}. We put result into {\em reached\_nodes}. After we get all variations we can wheter we have Cayley graph with diameter {\em n}. Graph is of diameter {\em n} only if we produced all nodes of Cayley graph. Our function checks all diameters up to {\em diameter}. If we algorithm fails to return diameter graph has bigger one. Algorithm doesn't deal with case of diameter 1 which can be done by checking degrees on each node on Cayley graph. This is special case for checking graphs with diameter $2$. Only after confirmation that our algorithm will produce all nodes we have to generate graph and check degrees on nodes.
...
%%% TU POKRACUJ S Adaptive search algorithm
