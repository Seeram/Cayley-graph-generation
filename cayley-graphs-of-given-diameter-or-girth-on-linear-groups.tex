...
All of our Cayley graphs which are generated through its symmetric sets were choosed randomly. As mentioned earlier choosing random symmetric sets consits of choosing elements of group with its inverses over finite field arithmetic. In our implementation all elements of group were precomputed. Every element $A$ had its inverse computed by formula:
\begin{align}
	\frac{1}{det(A)}adjugate(A) = A^{-1}
\end{align}
We also keept all involutions apart in case of choosing last element for sets with odd size. 

We present algorithm for generation of Cayley graph in pseudocode. ~\\

\lstinputlisting[language=Perl, frame=sinlge]{cayley_graph_generation.pl} 

On lines $8,10,12$ we denoted by $[ r ]$ putting resulting matrix into so called associative array also known as dictionary or hash array. It's worth mentioning that there exists perfect hash function for matrices over finite fields for such purposes. Algorithm uses $stack[]$ for storing matrices which weren't multiplied over generating set while $generating\_nodes[]$ to keep them unique to keep memory overhead low at cost of computation of hash. 

Precomputations could be made for finding inverse and creating Cayley table for matrix multiplications, making generation of graph and diameter check matter of accessing memory. While this implementation should give faster performance results, it was considered at later stages and was not implemented. Precomputations of Cayley table are limited by size of RAM memory and possible only in smaller groups. 

To check diameter of graph one has to make only computations on generating set. To check wheter graph has diameter $k$ it's required least if graph has not of diameter $k-1$. To find unknown $k$ one has to iterate through all $k \in {1 \dots}$ diameters. We didn't use algorithm for special case of $k=1$ which was checked only by checking degree of all nodes in graph.  ~\\

\lstinputlisting[language=Perl, frame=sinlge]{check_diameter.pl}

\subsection{Computer search of graphs with diameter}

Here we present results of computer seach of graphs. We have choosed two approaches in order to find graphs with diameter two. In early stages of program we have tried to find graphs with generating fixed size of graphs on degree $d$. Later on we used adaptive algorithm to search any graph of diameter two with ability to improve its results if it's feasible. 

Search with fixed size of generated graphs was used on two smaller groups $SL(2,5)$ and $SL(2,7)$. It's asssumed that Cayley graph will yield all elements of groups. Here we present table of results for $SL(2,5)$ with Moore bound for parameters $k=2$ and $|SL(2,5)| = 120$ is $d=11$, therefore search was done on degree $12$. $50000$ random graphs were genreated on each degree with following results: \\
...
Same approach was used on graphs in $SL(2,7)$ with order of $336$ with Moore bound on $d=19$. We have searched $150000$ graphs on each degree from $20$ to $30$ with only $10$ graphs found on $d=30$. 

By adaptive algorithm we mean automatized search of graphs in group. Program needs to be set up with starting size of generating set, time limit spent per degree, maximum of generated graphs per degree and number of parallel processes. In order to find some graph of given diameter program's tries to find any graph on degree. In case time limit is up it's continuing on set with bigger size making found graph further from Moore bound with bigger probability to find some. After first successfuly found graph search continues on smaller generating size with bigger time limit.
...
For implementation of our algorithms we choosed {\em Perl} programming language. Perl is high-level, general-purpose, interpreted programming language with great documentation and online community maintaining easy to use libraries. While Perl being high-level programming language multiplication of matrices in our program were actually implemented in C programming language implementation in choosed language more suitable. For this purpose we have used library called PDL or perl data language available at {\em CPAN}~\cite{CPAN}.

\section{The degree/girth problem}
Finding graphs with smallest possible order denoted $n_{d,g}$ with given degree $d$ and girth $g\geq3$ is known as {\em degree/girth problem}. Motivation for findind such graphs could arise from constructing graphs with no cycles with length $g$ or less and its similarity to {\em degree/diameter problem}. Problem was studied first by Biggs ~\cite{Biggs} on regular graphs. 
\subsection{Moore bound}
For odd $g$ we have Moore bound denoted by $M_{d,g}$ 
\begin{equation*}
...
For odd $g$ Moore bound constructed same as Moore bound for degree/diameter problem with $k=(g-1)/2$. For even $g$ we start with edge and from incident vertices we continue until we reach depth $(g-2)/2$. 
\subsection{Moore graphs}
Graphs with Moore bound equal to $M(d,g)$ are called Moore graphs or cages. We list all ceses with $n_{d,g} = M_{d,g}$ below:

\begin{itemize} 
	\item{\makebox[2cm]{For $d=2$,\hfill}$g \geq 2$ - circles}
	\item{\makebox[2cm]{For $g=3$,\hfill}$d \geq 22$ - complete graphs}
	\item{\makebox[2cm]{For $g=5$,\hfill}$d \geq 2$ - complete bipartite graphs}
	\item{\makebox[2cm]{For $g=5$,\hfill}$d=2$ - circle with length $5$}
	\item[]{\makebox[2cm]{\hfill}$d=3$ - Peterssen graph}
	\item[]{\makebox[2cm]{\hfill}$d=7$ - Hoffman-Singleton graph}
	\item[]{\makebox[2cm]{\hfill}$d=57$ - possible, graph is unknown }
	\item{\makebox[3cm]{For $g \in {6,8,12}$,\hfill}if $d-1$ is prime factor}
\end{itemize}

\begin{figure}[!ht]
	\centering
	\includegraphics[scale=0.15]{Heawood_graph.png}
	\caption{Heawood graph is Moore graph with $d=3$ and $g=6$ }
\end{figure}

\subsection{Example of Cayley graph}
Here we present example of Cayley graph on group $SL(2,3)$ with order $|SL(2,3)|=24$

\subsection{Generation of cayley graphs with given girth}
Same as for diameter we check girth of graph making computations on generating set. Checking if girth $g$ on graph we have to confirm that graphs doesn't have cycle with length $3 \leq n \leq g-1$. Here we present algorithm used for computation of girth:
...
In our work we discussed problems of construction of graphs with restriction on {\em degree/diameter} and {\em degree/girth}. We discussed theoretical bounds on both problems called after Moore and special cases of graphs reaching Moore bound. We introduced Cayley graphs as special case with symmetric properties. Symmetric properties allowed making them ideal for computation of our restrictions of random graphs. Considering Cayley graphs generated from special linear group over Galois field we have tried to find best possible resutls with computer search with some graphs found in smaller groups. We described implementation and our approach in search for such graphs with our results.   

\section{Abstract}

This bachelor thesis discuss two problems concerning construction of graphs with given restriction, namely {\em degree/diameter} and {\em degree/girth} problem. In our work we describe both problems, its theoretically possible bounds and some research results with focus on special case of Cayley graphs. We present algorithm for generating Cayley graph and exploitation of symmetry of Cayley graphs with computation of it's girth and diameter just on its symmeteric generating set. We also triedto find graphs as close to theoretical bound of both problems as possible and we list some of results in our randomized search in Special linear group consisting of linear transformations over Glois field with smaller prime orders.
